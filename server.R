...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$Sel_municipio, {
    ...
    output$ideb <- renderText(format(dados_municipio$ideb_valido[dados_municipio$mun == input$Sel_municipio], nsmall = 1, big.mark = ".", decimal.mark = ","))
    output$pop <- renderText(format(dados_municipio$pop_2019[dados_municipio$mun == input$Sel_municipio], nsmall = 0, big.mark = ".", decimal.mark = ","))
    output$inv <- renderText(paste("R$", format(dados_municipio$inv_pc[dados_municipio$mun == input$Sel_municipio], nsmall = 2, big.mark = ".", decimal.mark = ",")))
    output$pib <- renderText(paste("R$", format(dados_municipio$pib_pc[dados_municipio$mun == input$Sel_municipio], nsmall = 2, big.mark = ".", decimal.mark = ",")))
